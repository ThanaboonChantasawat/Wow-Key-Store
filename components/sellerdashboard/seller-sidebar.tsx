...
import { BarChart3, Package, Box, AlertCircle, CreditCard, Store } from "lucide-react"
...
export function SellerSidebar({ activeSection, onSectionChange }: SellerSidebarProps) {
  const menuItems = [
  ...
  return (
    <aside className="w-72 flex-shrink-0">
      <div className="bg-white rounded-lg p-6 shadow-sm">
        ...
        <Button className="w-full bg-[#ff9800] hover:bg-[#ff9800]/90 text-white mb-6">+ เพิ่มรายการสินค้า</Button>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                  isActive ? "bg-[#fff3e0] text-[#ff9800] font-medium" : "text-[#292d32] hover:bg-[#f9fafb]"
                }`}
              >
